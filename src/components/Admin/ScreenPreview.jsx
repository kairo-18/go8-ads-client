...
function ScreenPreview({ screenRefreshTrigger }) {
                ...
                {data.screens.map((screen, index) => (
                    <div key={index} className="w-full border rounded-lg p-2 bg-[#f9f6f5] shadow-md flex flex-col justify-center items-center overflow-hidden h-70">
                        <div
                            ...
                            style={{ transform: "scale(0.25) scale(1)", transformOrigin: "center" }}
                        >
                        ...
                        <button className="-mt-5 text-center text-blue-500 underline" onClick={() => navigate(`/${screen.routeName}`)}>
                            {screen.name}
                        </button>
                    </div>
                ))}
